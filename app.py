...
#              text_color='white', size=(220, 0), font='Tahoma', key='song_name')]
# ]
...
currently_playing = [
    [sg.Text(text='Press play...', background_color='black', justification='center', size=(200, 0), text_color='white',
...
main = [
    ...
    [
        sg.Canvas(background_color='black', size=(110, 200), pad=(0, 0)),
        sg.Image(pad=(10, 0), filename=GO_BACK_IMAGE_PATH, enable_events=True,
                 size=(48, 48), key='previous', background_color='black'),
        sg.Image(filename=PLAY_SONG_IMAGE_PATH,
                 size=(100, 100), pad=(10, 0), enable_events=True, key='play', background_color='black'),
        ...
        sg.Image(filename=GO_FORWARD_IMAGE_PATH, enable_events=True,
                 size=(48, 48), pad=(10, 0), key='next', background_color='black'),
    ]
]
window = sg.Window('Spotify', layout=main, size=(
    495, 670), background_color='black', finalize=True, grab_anywhere=True, resizable=False)

directory = sg.popup_get_folder(
    'Select Music Directory', background_color='black', button_color='black', text_color='white',)
...
while True:
    ...
    if event == sg.WIN_CLOSED:
        ...
        if is_sound_playing():
            pause_sounds()
            window['play'].update(filename=PLAY_SONG_IMAGE_PATH)
        else:
            unpause()
            play_sound(songs_in_directory[current_song_index])
            update_song_display()
            window['play'].update(filename=PAUSE_SONG_IMAGE_PATH)
